...
- The local default Turbopack build was denied a worker port; webpack provided local production-build validation. Vercel subsequently built and deployed the change successfully with Turbopack on September 12, 2026.
- Production health, public routes, security headers, and the canonical redirect passed. The production crawler verified 28 URLs and eight unique 1200 × 630 PNG previews. Main-branch Quality and Security CI passed. See the [release record](releases/2026-09-12-sharing.md).
- Third-party live previews, physical-device native sharing/cancellation, and denied-clipboard behavior remain manual checks. No posts were published to social networks during validation.
